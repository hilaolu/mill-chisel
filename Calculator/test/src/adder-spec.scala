...
class SubtracterTest extends FlatSpec with ChiselScalatestTester with Matchers {
    behavior of ""

    it should "work" in {
        test(new Subtracter) { c =>
            c.reset.poke(true.B)
            c.clock.step()
            c.reset.poke(false.B)
            c.io.in.op_1.poke(1.U)
            ...
            for (i <- 0 until 10000) {
                ...
                val random_op_2 = r.nextLong.abs % (INT_MAX + 1)
                val random_result =
                    (random_op_1 - random_op_2 + INT_MAX + 1) % (INT_MAX + 1)
                println(
                  "let's test " + random_op_1 + "-" + random_op_2 + "=" + random_result
                )
                c.io.in.op_1.poke(random_op_1.asUInt(32.W))
