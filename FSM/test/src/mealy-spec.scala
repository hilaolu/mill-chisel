...
class MealyTest extends FlatSpec with ChiselScalatestTester with Matchers {
    behavior of ""
    ...
    it should "work" in {
        test(new MealyFSM) { c =>
            ...
            c.io.in.signal.poke(0.U)
            // c.clock.step()
            // ^ Note that for Mealy FSM there's no gap between the last input and output signal.
